...
class Drone:
    ...
    def _send_message(self):
        if self.enabled:
            self.socket.sendto(self._build_message(), ("192.168.1.1", 7099))
            self.send_interval = Timer(0.05, self._send_message)
            self.send_interval.start()
    ...
    def move_forward(self, speed=50, wait=0):
        self.move('forward', speed, wait)

    def move_backward(self, speed=50, wait=0):
        self.move('backward', speed, wait)

    def move_left(self, speed=50, wait=0):
        self.move('left', speed, wait)

    def move_right(self, speed=50, wait=0):
        self.move('right', speed, wait)

    def move_up(self, speed=50, wait=0):
        self.move('up', speed, wait)
    ...
    def move(self, direction, speed=50, wait=0):
        ...
        self.left_right = 128  # Reset left/right to neutral
        self.throttle = 128  # Reset throttle to neutral

    def start_video(self):
        self.stop_event = Event()
        self.video_thread = Thread(target=self._start_video)
        self.video_thread.start()
        time.sleep(3) # delay to allow the video to show up before any further control motion [e.g monitoring takeoff]
